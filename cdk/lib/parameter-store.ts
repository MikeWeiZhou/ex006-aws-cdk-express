...
export class ParameterStore {
   ...
   * @returns parameter value
   */
  getSecureString(scope: cdk.Construct, parameterName: string, parameterVersion: number): string {
  ...
  getSecureStrings(scope: cdk.Construct, parameterVersions: ParameterVersions): Parameters {
    const results: { [parameterName: string]: string } = {};
    for (const parameterName in parameterVersions) {
      results[parameterName] = this
        .getSecureString(scope, parameterName, parameterVersions[parameterName]);
    }
    return results;
  }
  /**
  ...
  getSecret(scope: cdk.Construct, parameterName: string): ecs.Secret {
    const ssmParam = ssm.StringParameter.fromStringParameterName(scope, parameterName, parameterName);
    return ecs.Secret.fromSsmParameter(ssmParam);
  }
  /**
   * Returns `SecureString` typed parameters from AWS Parameter Store wrapped as secret environment
   * variables.
